...
3. Double-click **`start.bat`** — otomatis `npm install` kalau belum pernah, lalu membuka **jendela baru** yang menampilkan log live service ini (persis seperti menjalankan `npm start` manual). **Biarkan jendela itu tetap terbuka** selama live berlangsung — menutupnya = mematikan service. `start.bat` juga otomatis menjalankan **queue worker Laravel** (`php artisan queue:work`) dan **web server GiftTokTok** (`php artisan serve`) di background tersembunyi, lalu membuka Chrome ke `http://127.0.0.1:8000` otomatis — tidak perlu setup Apache/XAMPP vhost sama sekali (vhost pernah terbukti gagal baca `.env` di sebagian komputer, `php artisan serve` tidak kena masalah itu). Queue worker yang memproses trigger Event Trigger tipe **like/chat** (lihat menu Event Trigger di Admin project). Semua butuh `php` ada di PATH Windows (cek: `php -v`), kalau belum ada akan muncul pesan gagal tapi listener gift tetap jalan normal.
...
6. Selesai live: double-click **`stop.bat`** untuk menghentikan listener, queue worker, dan web server sekaligus (atau tutup langsung jendela log listener-nya — tapi queue worker & web server tetap perlu dihentikan lewat `stop.bat` karena jalan tersembunyi tanpa jendela).
