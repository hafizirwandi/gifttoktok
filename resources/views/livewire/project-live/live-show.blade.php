    ...
    @else
        <div class="max-w-[430px] mx-auto">
            <div class="flex h-[50vh]">
                ...
                <div class="w-[46%] grid grid-cols-2 grid-rows-4 gap-1.5 p-1.5">
                    @foreach ($details as $detail)
                        @if ($detail['status'] === 'show')
                            <div wire:click="toggleClick({{ $detail['id'] }})"
                                class="relative rounded-lg overflow-hidden border-2 border-white/15 cursor-pointer">
                                ...
                                <div class="relative h-full flex items-center justify-center">
                                    @if ($detail['img_url'])
                                        <img src="{{ $detail['img_url'] }}" alt="{{ $detail['name'] }}" class="w-[42%] aspect-square rounded-full object-cover ring-1 ring-white/20">
                                    @else
                                ...
                                <div class="absolute top-1 left-1 flex items-center gap-1 bg-black/60 rounded-full pl-0.5 pr-2 py-0.5">
                                    <span class="w-3.5 h-3.5 rounded-full bg-sky-400 flex items-center justify-center flex-shrink-0">
                                        <svg viewBox="0 0 20 20" fill="currentColor" class="w-2 h-2 text-white">
                                ...
                                <!-- Mic mute: pojok kanan bawah -->
                                <div class="absolute bottom-1 right-1 w-5 h-5 rounded-full bg-black/60 flex items-center justify-center">
